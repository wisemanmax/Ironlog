...
|Metric           |Value                                                     |
|-----------------|----------------------------------------------------------|
|Frontend         |13,532 lines · single HTML file                           |
...
|Service Worker   |v7.5 · offline fallback · background sync · push hardening|
...
- CORS restricted to `ironlog.space` on all endpoints
...
├── index.html              — 13,532-line single-file React PWA
├── sw.js                   — Service Worker v7.5
...
|`LATEST_APP_VERSION`  |Current version string (e.g. `7.5`)              |
...
|v7.5   |**iMessage-style Messages tab:** corner radii, date separators, friend avatar, image bubbles, emoji reactions, quick reply chips, push notification prompt, pill textarea · **DM inbox:** sorted by last message time, unread badges, skeleton loading, search, pending requests banner · **Smart unread system (3 bug fixes):** watermark-based deduplication, merge-not-replace poll, cap at 200 messages · **Settings Advanced/Basic toggle** persisted to localStorage · **Push notifications wired end-to-end:** VAPID server, notify_dm with 60s rate limit, SW deep-link open · **Bug fix pass (5 fixes):** UndoToast countdown bar now drains in real-time · handleNotifAccept registers Web Push subscription · duplicate notification modal removed · MessageBanner DM tap routes to Messages tab · MessagesTab double-padding removed|
